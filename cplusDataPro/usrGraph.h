...
/** 私有宏------------------------------------------------------------------------*/
	...
	#define macroGpStkTmp     usrStackUnique  //没有重复数据
...
{
#if 0
	//vector<bool>vecLineState(m_numNode,false);  //用于标志边是否已经存在
	HeadNode<nodeDataType, infoType>*curHeadPtr = nullptr;
	int curIndex = 0, minLineIndex = 0;
	//声明最小权重的边
	int minWeightLine = GRAPH_MAX_WEIGHT_LINE;
	vector<vector<int>>vecIndexCache(m_numNode);  //用于存储最小边两边的结点索引值
	for (int loop = 0; loop < m_numNode; loop++)
	{
		for (curIndex = 0; curIndex < m_numNode; curIndex++)   //该循环使用宽度优先遍历，查找最小权重的边
		{
			curHeadPtr = m_vecHead[curIndex];
			//使用宽度优先遍历第一层，也就是遍历在邻接表中的一个链表，并返回最小边的头结点索引值
